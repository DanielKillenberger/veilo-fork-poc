> **Archived working notes from July 10, 2026, not current findings.** The corresponding source fixes are `af9dc7f` (nullifiers) and `1768b9c` (prefunding amount binding). The extra suggested cap on returned proceeds was not validated in this review and could interfere with legitimate profitable positions. See [current disclosure](../README.md).
